---
title: "Manual of comparing optimal subsampling with model robust optimal subsampling method for logistic regression (exponential distribution)"
output: 
  html_document:
      keep_md: yes
---
...
* Based on the real model of the logistic regression data we have four folders and in each folder their respective Big Data sets are stored by using ***Checking_SP_Model_1_Exponential_Special.R***.
...
1. $\theta=[-2.0, 1.5, 0.3]$ generates data for Model 1 of $x_1,x_2$.
2. $\theta=[-2.0, 1.7,-1.2, 0.2]$ generates data for Model 2 of $x_1,x_2,x^2_1$.
3. $\theta=[-2.0,-1.3, 1.9, 0.9]$ generates data for Model 3 of $x_1,x_2,x^2_2$.
4. $\theta=[-2.0, 1.9, 1.9, 0.9, 0.7]$ generates data for Model 4 of $x_1,x_2,x^2_1,x^2_2$.

* For each model structure covariate data was generated only through exponential distribution with no correlation. 
...
* Use **Articles.Rmd** to generate plots relevant to the paper as pdf files **Artilce_Plots/SMSE_MI_LR_TV_Exp_mMSE.pdf** and **Artilce_Plots/SMSE_MI_LR_TV_Exp_mVc.pdf**. 
